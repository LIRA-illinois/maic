...
class ForagingEnvWrapper(MultiAgentEnvWrapper):
    ...
    def __init__(
        ...
        env_id = "Foraging{4}-{0}x{0}-{1}p-{2}f{3}-v1".format(
            field_size,
            players,
            max_food,
    ...
    def step(self, actions) -> tuple[float, bool, dict]:
        ...
        if self.is_print:
            ...
            pickle.dump(
                ...
                open(
                    os.path.join(
                        self.render_path, f"info_step_{self._episode_steps}.pkl"
                    ),
                    "wb",
                ),
            )
        ...
        if self.need_render:
            ...
            plt.axis("off")
            fig.savefig(
                os.path.join(self.render_path, f"image_step_{self._total_steps}.png"),
                bbox_inches="tight",
                dpi=600,
            )
